...
- [`ising_sw_plus.py`](ising/ising_sw_plus.py) — Swendsen–Wang sampler on the `N x N`
  square lattice with `+` boundary conditions.
...
- [`gff_fft_zero.py`](gff/gff_fft_zero.py) — GFF with zero b.c. on the unit square.
...
Scripts for the random current, the double random current and the height
function associated to the Ising model. For more details on the construction (and history) of the coupling, refer
to Sections 2.1–2.2 of [arXiv:2602.05886](https://arxiv.org/abs/2602.05886) and
references therein.

- [`currents.py`](coupling/currents.py) — Single and double currents, built out of agreement edges of the Ising models and independent Bernoulli percolation sprinkling. Returns each current's trace, odd part and even part. Also samples the coupled doubled-FK representation `omega`.
...
Definition of a `Percolation` class (of more general interest) to aid plotting, moving
between primal and dual models, and finding the cluster of a given point. Its `__main__` block simulates and plots Bernoulli percolation.
